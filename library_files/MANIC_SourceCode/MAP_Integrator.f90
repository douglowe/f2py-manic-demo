...
MODULE MAP_Integrator

  USE MAP_Parameters, ONLY: NVAR, NFIX, NSPEC, LU_NONZERO
  USE MAP_Global
  USE MAP_Monitor
  IMPLICIT NONE
  PUBLIC
...
!~~~>  Statistics on the work performed by the Rosenbrock method
  INTEGER, PARAMETER :: Nfun=1, Njac=2, Nstp=3, Nacc=4, &
                        Nrej=5, Ndec=6, Nsolint=7, Nsng=8, &
...
!~~~>  check if any of the solution is negative
   Err = zero
   negcheck: do j=1,n
	!if(abs(Ynew(j)).gt.1d0) write(6,*) 'Ynew(',j,'): ', Ynew(j)
   	if(Ynew(j).lt.0d0)then
		!write(6,*) 'neg Ynew(',j,'): ', Ynew(j)
		!if(Ynew(j).gt.(-1d-50))then
		!    !write(6,*) 'zero'
		!    !Ynew(j) = 0d0
		!else
		!    Err = 2d0
		!    Hnew = H*0.1d0 ! reduce timestep by factor of 10
		!    !exit negcheck 
		!endif
		!pause
	endif
...
!   write(6,*)
   !pause
   !write(6,*) 'Ynew(63): ', Ynew(63)
   !write(6,*) 'Ynew(65): ', Ynew(65)
   !if(Err.eq.2d0) pause

   if(Err.eq.zero) then ! if we've no negative masses then 
...
!~~~>  Check the error magnitude and adjust step size
   ...
   END DO UntilAccepted
	...
	!write(6,*) ' Step = ', ISTATUS(Nstp), '  DT = ', Direction*H, '  T = ', T
	!if(Y(ind_BIN2Hplu).gt.0d0) write(6,*) ' Step = ', ISTATUS(Nstp), '  DT = ', Direction*H,&
	!' pH = ', -log10(Y(ind_BIN2Hplu)/(6.0221367d23*water_content(2)*1e-3))
	!pause
...
! Local variables
   ...
   DO i=1,N
     ...
     !if(((Yerr(i)/Scale)**2).gt.100) write(6,*) 'Err(',spc_names(i),')= ',(Yerr(i)/Scale)**2
   END DO
   Err  = SQRT(Err/N)
...
!~~~> InOut variables
   ...
   ISTATUS(Nsolint) = ISTATUS(Nsolint) + 1
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 USE MAP_Parameters, ONLY: NVAR, LU_NONZERO
 USE MAP_Global, ONLY: FIX, RCONST, TIME
 USE MAP_Function, ONLY: Fun
 USE MAP_Rates
!~~~> Input variables
...
!~~~> Local variables
   ...
   call aerosol_bin_sort(C(NVARST:NVAR),C(NFIXST:NSPEC),full=.true.) !calculate aerosol microphysics
   call Update_RATE_CONVERSION()
   call Update_MODEL_TRAJ_DATA()
   CALL Update_RCONST(C(NVARST:NVAR),C(NFIXST:NSPEC))
   CALL Fun( Y, C(NFIXST:NSPEC), RCONST, Ydot )
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 USE MAP_Parameters, ONLY: NVAR, LU_NONZERO
 USE MAP_Global, ONLY: FIX, RCONST, TIME
 USE MAP_Jacobian, ONLY: Jac_SP, LU_IROW, LU_ICOL
 USE MAP_LinearAlgebra
 USE MAP_Rates
!~~~> Input variables
    REAL(kind=dp) :: T, Y(NVAR)
...
!~~~> Local variables
    ...
    call Update_MODEL_TRAJ_DATA()
    CALL Update_RCONST(C(NVARST:NVAR),C(NFIXST:NSPEC))
    CALL Jac_SP( Y, C(NFIXST:NSPEC), RCONST, Jcb )

    TIME = Told

END SUBROUTINE JacTemplate

END MODULE MAP_Integrator
